...
public class CaptchaSession implements Serializable {
  ...
  public CaptchaFrontEndData start(int optionCount) {
    ...
    this.captchaSessionInfo =
        new CaptchaSessionInfo(fieldName, validChoice.getObfuscatedName(), audioFieldName, audioOption, choices);
    ...
    CaptchaFrontEndData frontendData =
        new CaptchaFrontEndData(validChoice.getName(), fieldName, frontEndOptions, audioFieldName);
  ...
  public CaptchaValidationResult validate(CaptchaData captchaData) {
    ...
    if (receivedName == null) {
      ...
      return validateAudio(captchaData.getValue()) ? CaptchaValidationResult.VALID_AUDIO
          : CaptchaValidationResult.FAILED_AUDIO;
    } else if (receivedName.equals(imageFieldName)) {
      return validateImage(captchaData.getValue()) ? CaptchaValidationResult.VALID_IMAGE
          : CaptchaValidationResult.FAILED_IMAGE;
    } else {
      log.warn("Invalid response, neither audio fieldname {} nor image fieldname {} found in params {}", audioFieldName,
          imageFieldName, captchaData);
